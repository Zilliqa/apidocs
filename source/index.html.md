...
Returns the mining nodes (i.e., the members of the DS committee and shards) at the specified DS block. Nodes owned by Zilliqa Research are omitted. The `dscommittee` has no `size` field since the committee size is fixed for a given network.
